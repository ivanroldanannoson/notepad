...
import { PanelLeftClose, PanelLeftOpen, Files, Search, Settings, ChevronRight, X } from 'lucide-react';
...
export default function Sidebar({
    ...
    return (
        <>
            ...
            <div className={`
                flex flex-col border-r transition-all duration-300 ease-in-out shrink-0

                /* Mobile: overlay mode */
                ${isOpen
                    ? 'fixed inset-y-0 left-0 z-50 w-[280px] md:relative md:w-64 md:z-auto animate-slideInLeft md:animate-none'
                    : 'flex w-10 md:w-12'
                }
            ...
            `}>
                ...
                {/* Sidebar Activity Bar Icons (when closed — desktop only) */}
                {!isOpen && (
                            ...
                            <div className="mt-1">
                                {tabs.map(tab => (
                                    <div
                                        ...
                                        className={`group flex items-center gap-2 px-4 py-2 md:py-1.5 cursor-pointer text-[13px] border-l-2 transition-colors ${activeTabId === tab.id
