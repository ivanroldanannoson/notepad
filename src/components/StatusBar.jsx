...
export default function StatusBar({ cursorPos, activeTab, isDark, settings, onOpenSettings }) {
    ...
    return (
        <div className={`h-[26px] flex items-center justify-between px-2 sm:px-4 text-[10px] sm:text-[11px] font-medium select-none shrink-0 ${isDark ? 'bg-[#007acc] text-white/90' : 'bg-[#f8f8f8] text-gray-500 border-t border-gray-200/60'
            }`}>
            ...
            <div className="flex items-center gap-2 sm:gap-4">
                ...
                {settings && <span className="hidden sm:inline">Spaces: {settings.tabSize}</span>}
                <button
